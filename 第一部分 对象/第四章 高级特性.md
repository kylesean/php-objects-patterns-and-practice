...
```
首先创建一个抽象的父类 `DomainObject`。然后扩展它，创建两个子类分别是 User 和 Document，并将 create 方法声明为 `static`。 
...
代码合理正常，但它太繁琐了，我其实并不想为我创建的每个 DomainObject 子类写差不多样式的代码，怎么办呢？我会尝试将 create() 方法置于父类：
...
```
看上去合理多了有没有，代码一下整洁了不少。使用 self 作为类的引用，并用 new 关键字实例化。但实际上这段代码是错误的，运行时：
...
在 PHP5.3 之前都没有很好的解决方案，5.3之后引入了一个被称为 **延迟静态绑定** 的概念。该功能最明显的表现就是关键字： static。static 类似 self，但是它真正指向被调用者的类。这种情况下，Document::create() 就会创建新的 Document 对象。
...
static 关键字不仅可以用于实例化，像 self 和 parent 关键字一样，也可以用作静态方法调用的标识符，即便是在非静态上下文中。假如在我的父类 DomainObject 中有一个 `group` 的概念，默认情况下，继承的子类都属于 `default` 类，但是我希望我能够覆盖和控制某些个例，看示例：
